...
exports.get = function( req, res ){
  ...
  get_notifications({user_id:req.decoded.id, task_id:req.query.task_id}, function(notifications) {
...
function get_notifications(data, callback, errorCallback) {
  if(data.task_id) {
    ...
    queryString += " n.data as notification,";
    queryString += " n.task_id as key,";
    ...
    var queryString  = " SELECT "
    queryString += " n.task_id as `key`,";
    queryString += " ( SELECT count(nn.id) FROM notifications nn WHERE nn.task_id = n.task_id ) as count,";
    queryString += " MAX(n.updatedAt) as updatedAt ";
    queryString += " FROM notifications n ";
    queryString += " WHERE n.user_id = "+data.user_id;
    queryString += " AND n.status = 1";
